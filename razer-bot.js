  ...
  try {
    ...
    for (const acc of allAccounts) {
      ...
      try {
        ...
        const _rjRow = order.orderItemId
          ? db.exec('SELECT razer_jobs FROM order_items WHERE id=?', [order.orderItemId])
          : db.exec('SELECT razer_jobs FROM order_items WHERE order_id=? AND product_id=?', [orderId, order.packageId])
        let _prevJobs = []
        ...
        if (order.orderItemId) {
          db.run(
            'UPDATE order_items SET email_id_used=?, credit_deducted=COALESCE(credit_deducted,0)+?, razer_jobs=? WHERE id=?',
            [acc.id, goldToDeduct, JSON.stringify(_prevJobs), order.orderItemId]
          )
        } else {
          db.run(
