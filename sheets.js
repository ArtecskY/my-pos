...
function isEmailLike(fill_type) {
  ...
  if (!['UID', 'OTHER_UID', 'ID_PASS', 'RAZER_AUTO', '24PAY_AUTO', 'OOC_AUTO'].includes(fill_type)) return true
  return false
}
      ...
      itemDataList.forEach(({ item, data }, i) => {
        ...
        if (bundleEmailRows?.length) {
          ...
          rows.push([
            ...
            item.fill_type === '24PAY_AUTO' ? 'API' : item.fill_type === 'OOC_AUTO' ? 'OOC' : (item.email_used || (['UID', 'OTHER_UID'].includes(item.fill_type) ? String.fromCharCode(50,52) : '-')), // Email ที่ใช้
